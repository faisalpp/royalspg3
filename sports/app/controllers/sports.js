...
const getPreEvents = async () => {
  ...
  for (const k in sportslist) {
    ...
    await getUpcomingPage(k, moment().add(0, "days").format("YYYYMMDD"));
    await getUpcomingPage(k, moment().add(1, "days").format("YYYYMMDD"));
    await getUpcomingPage(k, moment().add(2, "days").format("YYYYMMDD"));
    await getUpcomingPage(k, moment().add(3, "days").format("YYYYMMDD"));
    // await getUpcomingPage(k, moment().add(4, "days").format("YYYYMMDD"));
    // await getUpcomingPage(k, moment().add(5, "days").format("YYYYMMDD"));
    // await getUpcomingPage(k, moment().add(6, "days").format("YYYYMMDD"));
    // await getUpcomingPage(k, moment().add(7, "days").format("YYYYMMDD"));
  }
};
...
const getLiveOdd = async () => {
  const liveMatches = await DB.w_sports.findAll({ where: { isPreMatch: false, period: { [Op.ne]: 'Finished' } } });
...
const saveLiveIds = async (SportId, liveIdParam) => {
  try {
    ...
    const exist = await DB.w_sports_live.findAll({ where: { SportId } });
    if (exist.length) {
      await DB.w_sports_live.update({ liveIds }, { where: { SportId } });
    } else {
      await DB.w_sports_live.create({ SportId, liveIds });
    }
    await DB.w_sports.update({ period: 'Finished' }, { where: { Id: { [Op.notIn]: liveIdParam }, isPreMatch: false, SportId } });
  } catch (error) {
...
const getOdds = (event_id, time = 20) => {
  return new Promise(async (resolve, reject) => {
    ...
    request(options, async (error, response, body) => {
      if (error) {
        ...
        if (body && body.success && body.results) {
          try {
            ...
            for (let result of results) {
              ...
              if (result.Scoreboard && result.Scoreboard.period) {
                if (result.Scoreboard.period == 'Finished') {
                  return await deleteEvent(result.Id);
                } else {
                  obj = { period: result.Scoreboard.period }
                }
              }
              ...
              await DB.w_sports.update({ ...result, Markets, ...obj }, { where: { Id: result.Id, period: { [Op.ne]: 'Finished' } } });
            }
          } catch (error) {
...
const getOverUnder = ({ t_score, handicap, oddType }) => {
  handicap = Number(handicap);
  console.log(t_score, handicap, oddType)
...
const getScores = ({ SportId, scores, ss }) => {
  let h_score = 0, a_score = 0, t_score = 0, h_score_f = 0, a_score_f = 0;
...
const football = async (bet, data) => {
  try {
    ...
    const h_score_s = h_score - h_score_f, a_score_s = a_score - a_score_f;
    let betResult = '';
    switch (marketType) {
      case '3way':
